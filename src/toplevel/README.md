...
| {% if release.dual_url %}[{{ release.label }}]({{ release.dual_url }}/README.md){% endif %} | {% if release.dir %}[{{ release.label }}](releases/{{ release.dir }}/README.md){% endif %} | {% if loop.index >= 4 %}⚠️ Prebuilt will be removed in {{ release.removal_month }} {{ release.removal_year }}.{% else %}✅ Prebuilt available.{% endif %} |
{% endfor %}| {% if dual_url %}[Earlier/Custom]({{ dual_url }}/tree/master/packer/v1){% endif %} | [Earlier/Custom](./packer/v1) | For earlier MATLAB releases, you must build your own machine image. |
